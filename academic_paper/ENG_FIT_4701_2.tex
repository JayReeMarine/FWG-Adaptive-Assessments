...
LLM-based method received higher composite ratings for Relevance and Quality, while the Clarity difference was
smaller. Qualitative responses highlighted personalisation
...
perceived relevance, clarity, and quality as study-specific evaluation
dimensions derived from the COSMIN content-validity framework
\cite{terwee2018}, treating them as practical indicators of whether
adaptive questioning feels more responsive to a user's changing context.
Despite this promise, the use of LLMs in
...
protocol \cite{maatoug2021}. Recent work has also explicitly called for
...
therapeutic dialogue because they elicit structured self-reports through clear
prompts, so conversational LLM risks translate, but do not transfer
directly, into questionnaire design constraints.

Despite this growing body of work, to our knowledge, no prior study has directly compared
...
for evaluating content validity. To our knowledge, no prior study has directly
...
COSMIN-grounded evaluation, which is the gap this study addresses.
...
production system, so architectural choices, including a shared interface,
fixed persona, and identical health domains, are driven by the comparison
design.

The baseline question bank was developed during the first project phase
through a review of validated and research-informed health questionnaire
items, including mental health instruments such as the PHQ-9
\cite{kroenke2001} and GAD-7 \cite{spitzer2006}. The item set was refined
through consultation with study partners and a Monash University
researcher with expertise in survey design and human data collection,
which informed wording improvements and helped keep the assessment
concise. These
expert-informed baseline items form the rule-based question set that the
LLM-based method adapts in this study.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.80\columnwidth]{figures/methodology_diagram.jpg}
  \caption{Research methodology overview. The project progressed through eight phases across two semesters: literature review, questionnaire design, prototype implementation, survey design, ethics approval, survey conduct, data collection, and quantitative and qualitative evaluation.}
...
Flash API \cite{geminiteam2025}. Before final implementation, the team conducted an informal
...
measurement-validity considerations from prior work, including evidence
that PHQ-9 responses can exhibit non-invariance across demographic groups
\cite{baas2011} and that questionnaire wording must account for cultural
context \cite{wild2005}. The East Asian international student profile
therefore represents a demographic for whom culturally adapted wording
(e.g., coursework stress, adjusting to a new country) is particularly relevant.
...
accordingly, following the ``if--then'' paradigm identified as dominant in
...
minute session. First, during \textbf{Orientation}, a user guide introduced
the prototype and the Sophie persona, instructing participants to evaluate
from Sophie's perspective. Second, during the \textbf{Rule-based check-in},
participants completed Sophie's Month~2 mental health check-in via the
branching engine, where the functional-impact item remained hidden because
the threshold was not met (Table~\ref{tab:comparison}). Third, during the
\textbf{LLM-based check-in}, participants completed the same Month~2
check-in via the LLM method, with identical interface layout. Finally,
participants completed a \textbf{Feedback survey} on an external Google Form
...
sizes were computed using Cohen's $d_z = t / \sqrt{n}$ \cite{lakens2013} to
characterise the practical magnitude of observed differences. Item-level
tests are reported descriptively; interpretation focuses on the three
pre-specified composite dimensions. Internal consistency was acceptable
across the method-specific three-item composites (Cronbach's
$\alpha=.74$--$.87$). As a robustness check for the ordinal Likert data,
Wilcoxon signed-rank tests were also computed for the three composite
differences. For qualitative
analysis, one researcher coded all open-ended responses inductively
following Braun and Clarke's thematic analysis approach \cite{braun2006},
and a second researcher reviewed the resulting code map for consistency
before recurring themes were grouped thematically.
...
\begin{table}[t]
\caption{Composite Likert scores (mean $\pm$ SD) with paired $t$-test results ($n = 40$, $df = 39$). Effect size $d_z = t/\sqrt{n}$.}
\label{tab:quant}
\centering
...
endorsement rather than mutually exclusive preference. To distinguish
directional endorsement from non-directional responses, participants who
endorsed both methods ($n=22$) and those who selected neither ($n=2$)
are reported together as no strong single-method preference ($n=24$,
60.0\%). Among directional endorsements, the LLM-based method was
selected by 17 participants (42.5\%), compared with eight (20.0\%) for
the rule-based method.

These results show that many participants did not express a single-method
preference, while directional endorsements favoured the LLM-based
version. The large number of participants selecting both methods suggests
that the rule-based method was not perceived as poor; rather, the
LLM-based method appeared to add personalisation and awareness of prior
history. Figure~\ref{fig:satisfaction} illustrates the distribution.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.9\columnwidth]{figures/satisfaction.pdf}
  \caption{Participant satisfaction endorsements (multi-select, $n=40$). Participants who endorsed both methods or neither are combined as ``no strong preference.'' Among directional endorsements, the LLM-based method was preferred roughly 2:1 over the rule-based method.}
...
codes into higher-level themes mapped to the three COSMIN dimensions. A
second researcher reviewed the resulting code map to verify consistency.
Table~\ref{tab:qual_preference} presents the thematic codes organised by
COSMIN dimension and method preference. Relevance-related themes were the
most frequent: personalisation and tailoring to context (27.5\%) and
awareness of history or change over time (15.0\%) were both associated
with the LLM-based method. Clarity themes were more mixed, as some
participants valued the LLM method's brevity (7.5\%), while others found
the rule-based method more straightforward (10.0\%) or noted wording
ambiguity in both methods (7.5\%). Quality themes favoured the LLM
method for depth and meaningfulness (10.0\%) and a more natural feel
(5.0\%).

These patterns indicate that participants valued the direction of
LLM-based adaptation, particularly for contextual relevance, while also
showing that clarity and perceived quality remained important design
considerations.

\begin{table*}[t]
\caption{Thematic coding of open-ended responses, by COSMIN dimension and method preference ($n=40$). Codes are non-mutually-exclusive; percentages are of the 40 valid respondents.}
\label{tab:qual_preference}
\centering
\small
\begin{tabular}{@{}llllp{4.8cm}@{}}
\toprule
\textbf{Main Theme} & \textbf{Subcategory (Code)} & \textbf{Favours} & \textbf{$n$ (\% of 40)} & \textbf{Example Quote} \\
\midrule
Relevance & Personalisation \& tailoring to context       & LLM     & 11 (27.5\%) & ``better tailored to my personal situation'' \\
Relevance & Awareness of history / change over time        & LLM     &  6 (15.0\%) & ``linked back to my month~1 questions'' \\
\addlinespace
Clarity   & Brevity / ease of answering                   & LLM     &  3 (7.5\%)  & ``I like the LLM because it is shorter'' \\
Clarity   & Straightforward / predictable                  & Rule    &  4 (10.0\%) & ``Rule-based because it is straightforward'' \\
Clarity   & Wording / instruction ambiguity                & Neither &  3 (7.5\%)  & ``confused how long I should check'' \\
\addlinespace
Quality   & Depth \& meaningfulness of assessment          & LLM     &  4 (10.0\%) & ``appropriate and meaningful'' \\
Quality   & Natural / human-like feel                      & LLM     &  2 (5.0\%)  & ``LLM feels more human-like'' \\
\bottomrule
\end{tabular}
\end{table*}
...
circumstances. Exploratory subgroup patterns were directionally
consistent with the main Relevance finding, with LLM-based scores higher
across all age, gender, health-app familiarity, and role groups. Because
several subgroup sizes were small, these patterns should be interpreted
as descriptive support rather than evidence of differential demographic
effects.
