...
The methodology comprises five components: (i)~implementation of two
...
Flash API. Before final implementation, the team screened three
candidates---Gemini, an OpenAI GPT model, and an Anthropic Claude
model---on the Sophie scenario, reviewing outputs for (i)~preservation
of intended mental health constructs, (ii)~useful personalised wording
rather than generic paraphrase, and (iii)~consistent JSON schema
compliance. This screening informed implementation selection rather
than constituting a formal benchmark. Gemini~2.5 Flash was selected
because it provided the strongest fit on these three criteria and was
further supported by four practical considerations aligned with the
prototype: a free-tier API quota that supports reproducibility, native
JSON response mode that maps directly to the questionnaire schema, low
inference latency suitable for real-time web use, and Google Cloud
integration alongside the Flutter and Supabase stack. We acknowledge
...
\begin{itemize}
  ...
  \item \textbf{Quality} (COSMIN \emph{comprehensiveness}): whether
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.95\textwidth]{figures/cosmin_combined.pdf}
...
downstream risk prediction. While this study did not directly measure
prediction outcomes, the observed pattern suggests that LLM-based
adaptation may contribute to better data collection by asking questions
that users perceive as genuinely connected to their circumstances.
...
open-source Llama) to test whether the observed advantages generalise
